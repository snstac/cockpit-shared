...
async function getUnitProperty(
    ...
    return (result[0] as { v?: unknown } | undefined)?.v;
}
...
export function ServiceManagementCard({ serviceName, onToast }: ServiceManagementCardProps) {
    ...
    async function runCtl(action: string, label: string) {
        try {
            ...
            if (/(\bcode \b4\b|exit status 4|exited with code 4)/i.test(detail)) {
                detail = _(
                    'Unit {0}.service was not found. Install the package that provides this unit (or create the unit file), then refresh this page.'
    ...
    return (
        <Card data-testid={`${serviceName}-service-card`}>
            <CardTitle>
            ...
            <CardBody>
                ...
                {unitNotFound && !error && (
                    <Alert
                        variant="warning"
                        title={_('Service unit is not installed')}
                    >
                        {_(
                            'systemd has no {0}.service. Install the package that provides this unit (see documentation), run daemon-reload if you added a unit file, then refresh this page.'
                        ).replace('{0}', serviceName)}
                    </Alert>
                )}
